...
class DossierMedicalController extends Controller
{
    ...
    public function create(Request $request, $patientId)
    {
        try {
            ...
            $existingDossier = DossierMedical::where('patientId', $patientId)->first();
            if ($existingDossier) {
                return response()->json(['status' => false, 'message' => 'Ce patient possède déjà un dossier médical'], 400);
            }
    ...
    {
        try {
            $dossierMedical = DossierMedical::where('patientId', $patientId)->first();

            if (!$dossierMedical) {
                return response()->json(['status' => false, 'message' => 'Dossier médical non trouvé pour ce patient'], 404);
            }
    ...
    {
        try {
            $dossierMedical = DossierMedical::with(['constantesVitales'])
    ...
    public function addConstanteVitale(Request $request, $patientId)
    {
        try {
            ...
            $data = $validator->validated();
            $data['dossierMedicalId'] = $dossierMedical->id; // Utiliser l'ID du dossier médical

            // Vérifier si une constante vitale existe déjà pour la période donnée
            $existingConstante = ConstanteVitale::where('dossierMedicalId', $dossierMedical->id)
    ...
    public function addRendezVous(Request $request, $patientId)
    {
        try {
            // Récupérer le dossier médical du patient
            $dossierMedical = DossierMedical::where('patientId', $patientId)->first();
